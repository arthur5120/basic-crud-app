...
export const UseGetList = async (setList : Function) => {
    ...
    const res = await fetch('http://localhost:3001/list-all')
    .then(res => res.json())        
    setList(res)
}
...
export const UseSaveChar = async (Char : CharType) => {    
    ...
    const res = await fetch('http://localhost:3001/save-one', {
        method:'POST',
...
export const UseUpdateChar = async (Char : CharType) => {     
    ...
    const res = await fetch(`http://localhost:3001/update-one/${Char?._id}`, {
        method:'PUT',
        body:JSON.stringify(Char),
        headers:{'Content-Type':'application/json'}
    })
...
export const UseDeleteChar = async (Char : CharType) => {  
    ...
    const res = await fetch(`http://localhost:3001/delete-one/${Char?._id}`, 
    {method:'DELETE',})
